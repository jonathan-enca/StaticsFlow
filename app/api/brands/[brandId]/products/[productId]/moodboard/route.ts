// POST /api/brands/[brandId]/products/[productId]/moodboard
// Uploads one or more moodboard images to R2 and appends their URLs to product.moodboardAssets.
// DELETE /api/brands/[brandId]/products/[productId]/moodboard — remove a specific moodboard asset URL
// Auth required — user must own the brand.
// Body (POST): multipart/form-data — one or more "file" fields (drag-and-drop, no URL input)
...
export async function POST(
  ...
  for (const file of files) {
    ...
    const key = productAssetKey(session.user.id, brandId, productId, "moodboard", randomUUID(), ext);
  ...
  const updated = await prisma.product.update({
    where: { id: productId },
    data: { moodboardAssets: { push: uploadedUrls } },
  });

  return NextResponse.json({ moodboardAssets: updated.moodboardAssets, uploaded: uploadedUrls }, { status: 200 });
}
...
export async function DELETE(
  ...
  const updated = await prisma.product.update({
    where: { id: productId },
    data: { moodboardAssets: product.moodboardAssets.filter((u) => u !== body.url) },
  });

  return NextResponse.json({ moodboardAssets: updated.moodboardAssets });
}
